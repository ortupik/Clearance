    ...
    if ($queryAvail) {
        ...
        if ($rows > 0) {
           ...
           if($avail > 0 ){
             ...
             $query = mysqli_query($db->connect(), "UPDATE `cleartable` SET `status` = 'Y'  WHERE `user` = 'Director' AND `user_id` = $user_id ") or die(mysqli_error($db->connect()));
            }
        }
  }
  }
  ...
  $hasBalance = "Student has  No balance !!";
   ...
   $query2 = mysqli_query($db->connect(), "Select * FROM `cleartable` WHERE `user_id` = '$user_id' AND user = 'Director' ") or die(mysqli_error($db->connect()));
    if ($query2) {
        ...
        if ($rows2 > 0) {
                $query = mysqli_query($db->connect(), "Select * FROM `cleartable` WHERE `user_id` = '$user_id' AND status = 'Y' AND user = 'Director'") or die(mysqli_error($db->connect()));
                   if ($query) {
                       $rows = mysqli_num_rows($query);
                       if ($rows > 0) {
