<?php
  ...
  $total_amount = 0;
   ...
   $query = mysqli_query($db->connect(), "Select *,(Select SUM(`amount`) FROM `damages` WHERE `user_id` = '$user_id') AS total_amount FROM `damages` WHERE `user_id` = '$user_id' AND `amount` > 0 GROUP BY `id` ") or die(mysqli_error($db->connect()));
    if ($query) {
...
<html lang="en">
    ...
    <body>
        ...
        <script src="./js/damages_payment.js"></script>
